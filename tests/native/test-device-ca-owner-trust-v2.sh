...
run_test() {
  ...
  log_info "List manufacturer vouchers"
  list_manufacturer_vouchers "${manufacturer_url}"

  log_info "Adding wrong Device CA certificate to owner"
  add_device_ca_cert "${owner_url}" "${manufacturer_crt}" | jq -r -M .
  ...
  log_info "Removing all the Device CA certificates from the owner server"
  ...
  log_info "Testing onboarding with no Device CA certificates on owner side (must succeed)"
  log_info "Run Device Initialization"
  guid=$(run_device_initialization)
  log_info "Device initialized with GUID: ${guid}"
  log_info "Sending Ownership Voucher to the Owner"
